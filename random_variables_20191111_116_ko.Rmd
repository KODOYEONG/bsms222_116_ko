...
```
We call this a **sampling model** since we are modeling the random behavior of roulette with the sampling of draws from an urn. The total winnings $S$ is simply the sum of these 1,000 independents draws:
```{r}
X <- sample(c(-1,1), n, replace = TRUE, prob=c(9/19, 10/19))
S <- sum(X)
S
...
```{r}
pbinom(n/2-1, size = n, prob = 10/10)
```
...
So in our roulette example, the standard deviation of the values inside the urn is: |1-(-1)|$\sqrt{10/19×9/19}$or:
```{r}
2 * sqrt(90)/19
```
...
$$μ = {1 \over n}\sum_{i=1}^n x_i$$

$$σ = \sqrt{{1 \over n}\sum_{i=1}^n (x_i-μ)^2}$$
...
$$\bar{X} = {1 \over N}\sum_{i=1}^N X_i,\quad s = \sqrt{{1 \over N}\sum_{i=1}^N (X_i-\bar{X})^2}$$
...
sqrt(mean((x-m)^2))
```
So be careful when using the sd function in R. However, keep in mind that throughout the book we sometimes use the sd function when we really want the actual SD. This is because when the list size is big, these two are practically equivalent since $\sqrt{(N-1)/N}\approx1$.