...
In this Chapter, we first discuss properties of a variety of distributions and how to visualize distributions using a motivating example of student heights. We then discuss the **ggplot2** geometries for these visualizations in Section $\color{blue}{9.16}$.
...
head(heights)
```
One way to convey the heights to ET is to simply send him this list of 1050 heights. But there are much more effective ways to convey this information, and understanding the concept of a distribution will help. To simplify the explanation, we first focus on male heights. We examine the female height data in Section $\color{blue}{9.14}$.
...
We discuss how to code histograms in Section $\color{blue}{9.16}$.
...
With the right argument, `ggplot` automatically shades the intersecting region with a different color. We will show examples of **ggplot2** code for densities in Section $\color{blue}{10}$ as well as Section $\color{blue}{9.16}$.
...
$Pr(a < x < b) = \int_{a}^{b} {1 \over \sqrt{2\pi}s}e^-{1 \over 2}({x-m \over s})^2 dx$
...
```
The pre-built functions `mean` and `sd` (note that for reasons explained in Section $\color{blue}{17.2}$, `sd` divides by `length(x)-1` rather than `length(x)`) can be used here:
```{r}
m <- mean(x)
s <- sd(x)
c(average = m, sd = s)
...
4. Plot the same quantiles versus the theoretical quantiles.

Let's construct a QQ-plot using R code. Start by defining the vector of proportions.
```{r}
p <- seq(0.05, 0.95, 0.05)
```
To obtain the quantiles from the data, we can use the `quantil`e function like this:
```{r}
...
qplot(theoretical_quantiles, sample_quantiles) + geom_abline()
```
The above code is included to help describe QQ-plots. However, in practice it is easier to use the **ggplot2** code described in Section $\color{blue}{9.16}$:
```{r}
...
We discuss how to make boxplots in Section $\color{blue}{9.16}$.
...
h <- data.frame("female_percentiles" = female_percentiles, "male_percentiles" = male_percentiles)
h
```
...
6. What proportion of countries in Europe have populations blow 14 million?
...
```{r}
ec <- mean(79<x & x<=81)
na <- pnorm(81, mean = m, sd = s) - pnorm(79, mean = m, sd = s)
ec
na
ec/na
```
...
```{r}
150 / (pnorm(6*12+8, mean = 69, sd = 3, lower.tail = FALSE) * 1000000000)
```
