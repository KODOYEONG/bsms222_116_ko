...
For example, if I have 2 red beads and 3 blue beads inside an urn$^{47}$ (most probability books use this archaic term, so we do too) and I pick one at random, what is the probability of picking a red one? Our intuition tells us that the answer is 2/5 or 40%. A precise definition can be given by noting that there are five possible outcomes of which two satisfy the condition necessary for the event “pick a red bead”. Since each of the five outcomes has the same chance of occurring, we conclude that the probability is .4 for red and .6 for blue.
...
This is the mathematical way of saying: the fact that  
$B$ happened does not affect the probability of $A$ happening. In fact, this can be considered the mathematical definition of independence.
...
When we have independent events, then the multiplication rule becomes simplier:

$Pr(A\ and\ B\ and\ C) = Pr(A)Pr(A)Pr(C)$
...
But to multiply like this we need to assume independence! Say the conditional probability of a man having a mustache conditional on him having a beard is .95. So the correct calculation probability is much higher:$1/10 \times 95/100$

The multiplication rule also gives us a general formula for computing conditional probabilities:

$Pr(B|A)= {Pr(A\ and\ B) \over Pr(A)}$
...
```
To get the conditional probability, we compute what fractino of these have a King in the second card:
```{r}
...
```
whicn uses `mean` instead of `sum` and is an R version of:

${Pr(A\ and\ B) \over Pr(A)}$
...
mean(stick)
switch <- replicate(B, monty_hall("stick"))
mean(switch)
```
...
$1 \times {364\over365} \times {363\over365} \cdots {365-n+1\over365}$
...
set.seed(1)
prop1 <- sample(balls, 10000, replace=TRUE) %>% table() %>% prop.table()
prop1
C <- prop1[1]
C
```
...
5. Two events $A$ and $B$ are indepent if $Pr(A\ and\ B)=Pr(A)P(B)$. Under which situation are the draws independent?
...
```{r}
1-(60/100*60/100*60/100*60/100*60/100*60/100*60/100)
```
...
```{r}
celtric_wins <- sample(c(0,1), 4, replace = TRUE, prob = c(0.6, 0.4))
```
The Celtics must win one of these 4 games.
```{r}
B <- 10000
Celtric_wins <- function(){
  celtric_wins <- sample(c(0,1), 4, replace = TRUE, prob = c(0.6,0.4))
 sum(celtric_wins) 
}
s <- replicate(B, Celtric_wins())
mean(s>=1)
```
...
```{r}
#5th round over: 1st loose, 2nd~5th win
c1 <- 1/2*1/2*1/2*1/2*1/2
#6th round over : 1st loose, 2nd~5th 3win 1loose, 6th win
c2 <- 1/2*ncol(combn(4,1))*(1/2)^4*1/2
#7th round over : 1st loose, 2nd~6th 3win 2loose, 7th win
c3 <- 1/2*ncol(combn(5,2))*(1/2)^5*1/2
#add all
c1+c2+c3
```
...
```{r}
prob_Cavs_win <- function(){
  Cavs_win <- sample(c(1,0), 6, replace = T, prob = c(0.5, 0.5))
  sum(Cavs_win)
}
B <- 10000
champ_win <- replicate(B,prob_Cavs_win())
mean(champ_win>=4)
```

12. Two teams, $A$ and $B$, are playing a seven game series. Team $A$ is better than team $B$ and has a $p > 0.5$ chance of whinning each game. Given a value $p$, the probability of winning the series for the underdog team $B$ can be computed with the following function based on a Monte Carlo simulation:
```{r}
prob_win <- function(p){
  B <- 10000
  result <- replicate(B, {
