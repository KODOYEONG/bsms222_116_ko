...
3.2.1 Objects
Suppose a high school student asks us for help solving several quadratic equations of the form ax2+bx+c=0. The quadratic formula gives us the solutions:
(-b + sqrt(b^2 - 4*a*c)) / (2*a) and
(-b - sqrt(b^2 - 4*a*c)) / (2*a)
which of course change depending on values of a, b and c. One advantage of programming languages is that we can define variables and write expressions with these variables, similar to how we do so in math, but obtain a numeric solution. We will write out general code for the quadratic equation below, but if we are asked to solve x2+x-1=0, then we define:
```{r}
a <- 1
b <- 1
c <- -1
...
We can also assign value using = instead of <-, but we recommend against using = to avoid confusion.
...
To see the value stored in variable, we simply ask R to evaluate a and it shows the stored value:
```{r}
a
```
A more explicit way to ask R to show us the value stored in a in using print like this:
```{r}
print(a)
```
We use the term object to describe stuff that is stored in R. Variables are examples, but objects can also be more complicated entitles such as functions, which are described later. 
...
We already used the install.packages, library, and Is functions. We also used the function sqrt to solve the quadratic equation above. There are many more prebuilt functions and even more can be added through packages. These functions do not appear in the workspace because you did not define them, but they are available for immediate use.
In general, we need to use parentheses to evaluate a function. If you type Is, the function is not evaluated and instead R shows you the code that defines the function. If you type Is() the function is evaluated and, as seen above, we see objects in the workspace.
Unlike Is, most functions require one or more arguments. Below is an example of how we assign an object to the argument of the function log. Remember that we earlier defined a to be 1:
```{r}
log(8)
log(a)
```
...
help("log")
```
For more functions, we can also use this shorthand:
```{r}
?log
```
...
```
For more advice, we highly recommend studying Hardley Wickham's style guide.

3.2.6 Saving your workspace
alues remain in the workspace until you end your session or erase them with the function rm.But workspaces also can be saved for later use. In fact, when you quit R, the programs asks you if you want to save your workspace. If you do save it, the next time you start R, the program will restore the workspace.
...
```{r}
## Code to compute solution to quadratic equation of form ax^2 + bx + c
...
We discuss more important features of vector after the next set of exercises.
Advanced:Mathematically, the value in pop are integers and there is an interger class in R. However, by default, numbers are assigned class numeric even when they are round integers. For example, class(1) returns numeric. You can turn them into class integer with the as.integer() function or by adding an L like this: 1L. Note the class by typing: class(1L)
...
```
It is a factor. Factors are useful for storing categorical data. We can that there are only 4 regions by using the levels function:
```{r}
...
mat[2, 3]
```
If you want to entire second row, you leave the column spot empty:
```{r}
mat[2, ]
```
...
mat[1:2, 2:3]
```
You can convert matrices into data frames using the function as.data.frame :
```{r}
as.data.frame(mat)
```
