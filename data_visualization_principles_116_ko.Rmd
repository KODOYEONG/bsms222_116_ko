...
We will be using this libraries:
```{r}
library(tidyverse)
library(dslabs)
library(gridExtra)
...
```{r}
west <- c("Western Europe","Northern Europe","Souther Europe","Northern America","Australia and New Zealand")
...
B. The axis does not star at 0. Judging by the length, it appears Trump received 3 times as many votes when, in fact, it was about 30% more.
...
```
Redefine the `state` object so that the levels are re-ordered. Print the new object `state` and its level so you can see that the vector is not re-ordered by the levels.
```{r}
reorder(state, rate)
```
...
#  geom_bar(stat="identity") +
#  coord_flip()
dat_new <- dat %>% mutate(state=reorder(state, rate))
dat_new %>% ggplot(aes(state, rate)) +
  geom_bar(stat="identity") +
  coord_flip()
```
...
7. Make a boxplot of the murder rates define as
```{r}
data("murders")
murders %>% mutate(rate = total/population*100000)
```
...
us_contagious_diseases %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>%
  filter(., disease == "Smallpox", weeks_reporting < 10, !is.na(rate)) %>%
...
us_contagious_diseases %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>%
  filter(state == "California", 10 <= weeks_reporting, !is.na(rate)) %>%
  ggplot(aes(year, rate, group=disease)) +
...
us_contagious_diseases %>%
  ...
  summarize(rate = count/population * 10000 * 52 / weeks_reporting) %>%
  mutate(rate = rate, !is.na(rate)) %>%
  ggplot(aes(year, rate, group=disease)) +
  geom_line(aes(color=disease))
```
...
#  group_by(disease, year, state) %>%
 # summarize(rate = count/population * 10000 * 52 / weeks_reporting) %>%
#  mutate(!is.na(rate)) %>%
#  ggplot(aes(year, state, fill = rate)) +
#  geom_tile(color = "grey50") +
#  scale_x_continuous(expand=c(0,0)) +
#  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9,"Reds"), trans = "logit") +
#  theme_minimal() + theme(panel.grid = element_blank()) +
#  ylab("") +
#  xlab("")
us_contagious_diseases %>%
