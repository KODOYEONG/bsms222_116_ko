...
For `geom+point` to run properly we need to provide data and a mapping. We have already connected the opject `p` with the `murders` data table, and if we add the layer `geom_point` it defaults to using this data. To find out what mappings are expected, we read the **Aesthetics** section of the help file `geom_point` help file:
```{r
> Aesthetics
> 
...
$r$, this line is defined by the formula: $y=rx$ , with $y$ and $x$ our axes: total murders and population in millions respectively. In the log-scale this line turns into: $log(y)=log(r)+log(x)$. So in our plot it’s a line with slope 1 and intercept $log(r)$. To compute this value, we use our **dplyr** skills:
```{r}
r <- murders %>% 
...
```{r}
murders %>% ggplot() +
  geom_point(aes(population, total), size=3, color="blue") +
  geom_text(aes(population, total, label = abb))
```
...
```{r}
murders %>% ggplot() +
  geom_point(aes(population, total, col=region), size=3) +
  geom_text(aes(population, total, label = abb))
```
