...
$$\begin{align}Pr(D=1|+)
&=\frac{P(+|D=1)\cdot P(D=1)}{Pr(+)}\\
&=\frac{Pr(+|D=1)\cdot P(D=1)}{Pr(+|D=1)\cdot P(D=1)+Pr(+|D=0)Pr(D=0)}\end{align}$$
...
We use a model to represent two levels of variability in our data. First, each player is assigned a natural ability to hit. We will use the symbol $p$ to represent this ability. You can think of as the batting average you would converge to if this particular player batted over and over again.
...
Now the second level of variability has to do with luck when batting. Regardless of how good the player is, sometimes you have bad luck and sometimes you have good luck. At each at bat, this player has a probability of success $p$. If we add up these successes and failures, then the CLT tells us that the observed average, call it $Y$, has a normal distribution with expected value $p$ and standard error $\sqrt{p(1-p)/N}$ with $N$ the number of a bats.

Statistical textbooks will write the model like this:

$$\begin{matrix}&p&\sim N(\mu,\tau^2)\\ Y|&p&\sim N(p,\sigma^2)\end{matrix}$$
...
$$\begin{matrix}&p&\sim N(.275,.027^2)\\ Y|&p&\sim N(p,.111^2)\end{matrix}$$
...
$$\begin{matrix}\mathrm{E}(p\ |\ Y=y)&=&B\mu + (1-B)y\\
&=&\mu +(1-B)(y-\mu) \\ \mathrm{with}B&=&\frac{\sigma^2}{\sigma^2+\tau^2}\end{matrix}$$

This is a weighted average of the population average $\mu$ and the observed data $y$. The weight depends on the SD of the population $\tau$ and the SD of our observed data $\sigma$. This weighted average is sometimes referred to as *shrinking* because it *shrinks* estimates towards a prior mean. In this case of José Iglesias, we have:

$\mathrm{E}(p\ |\ Y=.450)=B \times .275+(1-B)\times .450\\ =.275+(1-B)(.450-.275)\\B=\frac{.111^2}{.111^2+.027^2}=0.944\\ \mathrm{E}$
...
$\mathrm{Pr}(\mathrm{second\ case\ of\ SIDS}\ |\ \mathrm{first\ case\ of\ SIDS})<\mathrm{P}r(\mathrm{first\ case\ of\ SIDS)}$

b. Nothing. The multiplication rule always applies in this way:
$\mathrm{Pr}(A\ \mathrm{and}\ B)= \mathrm{Pr}(A)\mathrm{Pr}(B)$
...
2. Let’s assume that there is in fact a genetic component to SIDS and the probability of $\mathrm{Pr(second\ case\ of\ SIDS\ |\ first\ case\ of\ SIDS)}=1/100$, is much higher than 1 in 8,500. What is the probability of both of her sons dying of SIDS?
```{r}
1/8500*1/100
```
...
```{r}
#Pr(mother is a son-murdering psychopath | two children found dead with no evidence of harm) = Pr(two children found dead with no evidence of harm | mother is a son-murdering psychopath)*Pr(mother is a son-murdering psychopath)/Pr(two children found dead with no evidence of harm)
```
...
with A = two of her children are found dead with no evidence of physical harm and B = a mother is a son-murdering psychopath = 0.50. Assume that the rate of son-murdering psychopaths mothers is 1 in 1,000,000. According to Bayes’ theorem, what is the probability of $\mathrm{Pr}(B\ |\ A)$?
```{r}
0.50*1/1000000/(1/8500*1/100)
```
...
a. $\mu$ and $\tau$ are arbitary numbers that let us make probability statements about $d$.
...
8. The CLT tells us that our estimate of the spread $\hat{d}$ has normal distribution with expected value $d$ and standard deviation $\sigma$ calculated in problem 6. Use the formulas we showd for the posterior distribution to calculate the expected value of the posterior distribution if we set $\mu=0$ and $\tau=0.01$.
```{r}
mu <- 0
tau <- 0.01
sigma <- results$se
...
$$d \approx N(\mu, \tau^2)\ \mathrm{describes\ our\ best\ guess\ had\ we\ not\ seen\ any\ polling\ data}\\ \bar{X}\ |\ d \approx N(\mu, \tau^2)\ \mathrm{describes\ randomness\ due\ to\ sampling\ and\ the\ pollster\ effect}$$
...
posterior_mean + c(-1.96, 1.96)*posterior_se
```
The posterior probability $\mathrm{Pr}(d>0\ |\ \bar{X})$ can be computed like this:
```{r}
...
```
Now suppose we have $J=6$ data points from $I=5$ different pollsters. To represent this we now two indexes, one for pollster and one for the polls each pollster takes. We use $X_{ij}$ with $i$ representing the pollster and $j$ representing the $j$-th poll from that pollster. If we apply the same model, we write:
...
One way to interpret $b$ is as the difference between the average of all polls from all pollsters and the actual result of the election. Because we don’t know the actual result until after the election, we can’t estimate $d$ until after the election. However, we can estimate $b$ from previous elections and study the distribution of these values. Based on this approach we assume that, across election years, $b$ has expected value 0 and the standard error is about $\sigma_b=0.025$.
...
```
Here are the five closet races according to the polls:
```{r}
...
```{r}
cis <- polls %>% mutate(phat= (spread+1)/2, se= 2*sqrt(phat*(1-phat)/samplesize), lower = spread - qnorm(0.975)*se, upper = spread + qnorm(0.975)*se) %>%
