...
We want to know the probability that the interval $[\bar{X}-2\hat{SE},\bar{X}-2\hat{SE}(\bar{X})]$ contains the true proportion $p$. First, consider that the start and end of these intervals are random variables: every time we take a sample, they change. To illustrate this, run the Monte Carlo simulation above twice. We use the same parameters as above:
```{r}
p <- 0.45
N <- 1000
```
...
$Pr\left(-1.96 \leq \frac{\bar{X}-p}{\hat{SE}(\bar{X})}\leq1.96\right)$
...
```
Specially, we will use all the national polls that ended within one week before the election.
```{r}
library(tidyverse)
...
polls$se_hat <- sqrt(polls$x_hat * (1 - polls$x_hat) / N)
polls$lower <- c(polls$x_hat - 1.96 * polls$se_hat)
polls$upper <- c(polls$x_hat + 1.96 * polls$se_hat)
polls %>% select(pollster, enddate, x_hat, lower, upper)
```
...
```{r}
p <- 0.482
polls$hit <- ifelse(polls$lower<=p & polls$upper>=p,"true","fail")
polls
```
...
```{r}
polls$error <- polls$d_hat-0.021
polls %>% group_by(pollster) %>%
  ggplot(aes(pollster, error)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 4.6))
```

10. Redo the plot that you made for exercise 9, but only for pollsters that took five or more polls.
```{r}
pl2 <- as.data.frame(table(polls$pollster))
polls_freq <- pl2 %>% filter(Freq>5)
polls %>% filter(pollster %in% polls_freq$Var1) %>%
  group_by(pollster) %>%
  ggplot(aes(pollster, error)) +
  geom_boxplot() +
...
```
So we see that a large percent of men than women received awards:
```{r}
...
### 15.10.4 Toe odds ratio

An informative summary statistic associated with two-by-two tables is the odds ratio. Define the two variables as $X=1$ if you are a male and 0 otherwise, and $Y=1$ if you are funded and 0 otherwise. The odds of gettng funded if you are a man is defined:
...
Note that the log odds ratio is not defined if any of the cells of the two-by-two table is 0. This is because if $a,b,c$ or $d$ is 0, the $log\left(\frac{ad}{bc}\right)$ is either the log of 0 or has a 0 in the denominator. For this situation, it is common practice to avoid 0s by adding 0.5 to each cell. This is referred to as the *Haldane–Anscombe correction* and has been shown, both in practice and theory, to work well.
...
```{r}
athlete <- data.frame(record = c("win", "lose"), 
                     career = c(500*0.7, 500*0.3),
                     game = c(8, 9))
chisq_test <- athlete %>% select(-record) %>% chisq.test()
chisq_test$p.value
```
...
```{r}
odds_career <- with(athlete, (career[2]/sum(career)) / (career[1]/sum(career)))
odds_game <- with(athlete, (game[2]/sum(game)) / (game[1]/sum(game)))
odds_career/odds_game
log_or <- log(odds_career/odds_game)
se <- athlete %>% select(-record) %>%
  summarize(se = sqrt(sum(1/career) + sum(1/game))) %>%
  pull(se)
ci <- log_or + c(-1,1) * qnorm(0.975) * se
exp(ci)
```
...
```{r}
athlete %>% select(-record) %>%
  mutate(career = career*2, game = game*2) %>%
  chisq.test() %>% .$p.value
```
